<template>
  <ValidationObserver v-slot="{ handleSubmit }">
    <SfHeading v-e2e="'shipping-heading'" :level="3" :title="$t('Shipping')" class="sf-heading--left sf-heading--no-underline title" />
    <form @submit.prevent="handleSubmit(handleFormSubmit)">
      <div class="form">
        ...
        <ValidationProvider name="state" slim>
          <SfInput v-e2e="'shipping-state'" v-model="form.state" label="State/Province" name="state" class="form__element form__element--half" />
        </ValidationProvider>
      ...
      <div class="form">
        <div class="form__action">
          <SfButton v-e2e="'select-shipping'" v-if="!isFormSubmitted" :disabled="loading" class="form__action-button" type="submit">
            {{ $t('Select shipping method') }}
          </SfButton>
        </div>
      </div>
      <VsfShippingProvider v-if="isFormSubmitted" @submit="$router.push(localePath({ name: 'billing' }))" />
    </form>
  </ValidationObserver>
</template>
