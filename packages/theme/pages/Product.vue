    ...
    <div class="product">
      ...
      <div class="product__info">
        <div class="product__header">
          <SfHeading :title="productGetters.getName(product)" :level="3" class="sf-heading--no-underline sf-heading--left" />
        ...
        <div>
          <p class="product__description desktop-only" v-html="shortDescription"></p>
          <ProductAttributes :attributes="attributes" :selectedAttributeValue="selectedAttributeValue"></ProductAttributes>
          <SfAddToCart
        ...
        <LazyHydrate when-idle>
          <SfTabs :open-tab="openTab" class="product__tabs" @click:tab="changeTab">
            <SfTab title="Description">
              <div class="product__description" v-html="description"></div>
              <SfProperty v-for="(property, i) in properties" :key="i" :name="property.name" :value="property.value" class="product__property">
                <template v-if="property.name === 'Category'" #value>
                  <SfButton class="product__property__button sf-button--text" v-html="property.value"> </SfButton>
                </template>
                <template v-else #value>
                  <span class="sf-property__value" v-html="property.value"></span>
                </template>
              </SfProperty>
            </SfTab>
            <SfTab title="Read reviews">
              ...
              />
              <div v-show="!reviewsLoading" id="addReview">
                <ProductAddReviewForm :productId="productGetters.getId(product)" @add-review="successAddReview" />
              </div>
            </SfTab>
            <SfTab title="Additional Information" class="product__additional-info">
              <div class="product__additional-info">
                <p class="product__additional-info__title">{{ $t('Brand') }}</p>
                <p class="product__additional-info__paragraph">{{ manufacturer }}</p>
              </div>
            </SfTab>
          </SfTabs>
        </LazyHydrate>
...
export default {
  ...
  setup(props, context) {
    ...
    const changeTab = (tabNumber, callback) => {
      document.querySelector('#tabs').scrollIntoView({ behavior: 'smooth', block: 'end' });
    ...
    const changeNewReview = () => {
      changeTab(2, () => {
        setTimeout(() => document.querySelector('#addReview').scrollIntoView({ behavior: 'smooth', block: 'end' }), 500);
      });
    };
    ...
    const attributes = computed(() => productGetters.getAttributes(product.value));
    ...
    return {
      ...
      breadcrumbs: computed(() => productGetters.getBreadcrumbs(product.value)),
      // configuration,
      description: computed(() => productGetters.getDescription(product.value)),
      shortDescription: computed(() => productGetters.getShortDescription(product.value)),
      manufacturer: computed(() => productGetters.getManufacturer(product.value)),
      loading,
      // options,
      changeTab,
      openTab,
      ...
      productGallery,
      productGetters,
      properties: computed(() => productGetters.getProperties(product.value)),
      qty,
      relatedLoading,
...
.product {
  ...
  &__count {
    @include font(--count-font, var(--font-weight--normal), var(--font-size--sm), 1.4, var(--font-family--secondary));
    color: var(--c-text);
    text-decoration: none;
    margin: 0 0 0 var(--spacer-xs);
  }
  &__description {
    @include font(--product-description-font, var(--font-weight--light), var(--font-size--base), 1.6, var(--font-family--primary));
  }
  &__select-size {
    margin: 0 var(--spacer-sm);
    @include for-desktop {
